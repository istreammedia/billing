    ...
    private static final class SearchMapper implements RowMapper<SearchData> {
        ...
        public String searchSchema(final SearchConditions searchConditions) {
            ...
            String clientExactMatchSql = " (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                    ...
                    + " from m_client c join m_office o on o.id = c.office_id where o.hierarchy like :hierarchy and (c.account_no like :search or c.display_name like :search or c.external_id like :search or c.phone like :search or c.email like :search)) ";
            ...
            String deviceExactMatchSql = " (select distinct 'DEVICE' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                    + " , c.office_id as parentId, o.name as parentName "
                    + " FROM m_client c JOIN m_office o ON o.id = c.office_id  join  (select client_id,serial_no from b_allocation  union all  select client_id,serial_number from b_owned_hardware) hw" +
